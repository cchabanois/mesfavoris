...
import mesfavoris.texteditor.internal.IUIConstants;
...
public class PathPlaceholderCreationDialog extends StatusDialog {
	...
	private IStatus getPathStatus() {
		...
		if (path.length() == 0) {
			...
			status = new Status(IStatus.ERROR, Activator.PLUGIN_ID, "The path is invalid.");
		} else if (!new File(path).exists()) {
			status = new Status(IStatus.WARNING, Activator.PLUGIN_ID, "Path does not exist.");
		} else {
			status = new Status(IStatus.OK, Activator.PLUGIN_ID, "");
		}
		return status;
	...
	private IStatus getNameStatus() {
		...
		IStatus status;
		if (name.length() == 0) {
			status = new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Enter a variable name.");
			...
			status = new Status(IStatus.ERROR, Activator.PLUGIN_ID, "The variable name contains ':', '/' or '\'.");
		} else if (nameConflict(name)) {
			status = new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Variable name already exists.");
		} else {
			status = new Status(IStatus.OK, Activator.PLUGIN_ID, "");
		}
		return status;
